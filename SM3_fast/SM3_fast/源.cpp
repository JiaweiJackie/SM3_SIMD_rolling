...
static uint compress(unsigned char* m, uint hash[8])
{
	uint w68[68];
	uint w64[64];
...
void SM3(unsigned char* message, uint len, unsigned char* Hash)
{
	...
	for (i = 0; i < num; i++)
		compress(&buffer[i * 64], nHash);

	free(buffer);

	for (i = 0; i < 8; i++)
	{
		Hash[i * 4 ] = (unsigned char)((nHash[i] >> 24) & 0xFF);
...
int main()
{
	unsigned char message[256] = "Innovation and Entrepreneurship practice";
	printf("Message:", message);
	unsigned char digest[256] = "";
	uint len = strlen((const char*)message);
	...
	endTime = clock();
	printf("1 threads£º%.2f ms\n", (double)(endTime - startTime) );
	startTime = clock();
	for (int i = 0; i < 500; i++) {
		thread4(message, len, digest);
	}
	endTime = clock();
	printf("4 threads£º%.2f ms\n", (double)(endTime - startTime)/500);
	startTime = clock();
	for (int i = 0; i < 500; i++) {
		thread8(message, len, digest);
	}
	endTime = clock();
	printf("8 threads£º%.2f ms\n", (double)(endTime - startTime) / 500);
	printf("Digest:");
	printer(digest, 32);
	return 0;
}